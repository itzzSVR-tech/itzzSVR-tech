...
ROWS = [
    ("host",),
    ("kv", "Now", "B.Tech CSE Senior Student @ SRM IST RMP"),
    ("kv", "Prev", "SDE Intern @ Compassion AI | LLC"),
    ("kv", "Also", "SDE Intern @ iViewSense Pvt Ltd"),
    ("kv", "Edu", "B.Tech CSE, SRM IST RMP '27"),
    ("gap",),
    ("sec", "Stack"),
    ("kv", "Frontend", "React, Next.js, TypeScript"),
    ("kv", "Backend", "Node, NestJS, ElysiaJS"),
    ("kv", "AI", "Gemini, Vercel AI SDK, OpenAI"),
    ("kv", "Cloud", "Supabase, Vercel, Prisma ORM"),
    ("gap",),
    ("sec", "Highlights"),
    ("bul", "Tech Enthusiast 💻 | 150+ Projects"),
    ("bul", "10 blog posts · 6 on Substack, 4 on Dev.to"),
]
...
parts.append(f'<text x="{W/2}" y="{TITLEBAR_H/2 + 4}" fill="{MUTED}" font-size="12" '
             f'text-anchor="middle">svr@github: ~$ neofetch</text>')
...
for i, row in enumerate(ROWS):
    ...
    if kind == "host":
        inner = (f'<text x="{KEY_X}" y="{y:.1f}" font-size="14" font-weight="700">'
                 f'<tspan fill="{GREEN}">svr</tspan><tspan fill="{MUTED}">@</tspan>'
